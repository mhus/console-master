...
public class ConsoleInputDemo {
    ...
    private static String interpretEscapeSequence(byte[] buffer, int length) {
        ...
        if ((buffer[0] & 0xFF) == 27) {
            ...
            } else if (length > 2 && (buffer[1] & 0xFF) == '[') {
                // ANSI escape sequence
                return interpretAnsiSequence(buffer, length);
            }
        }

        // Control characters for single byte sequences
        if (length == 1) {
            int code = buffer[0] & 0xFF;
            if (code < 32 && code != 27) { // Exclude ESC which is handled above
    ...
    /**
     * Returns a description for special characters using numeric values to avoid conflicts.
     */
    private static String getCharacterDescription(char ch) {
        int code = (int) ch;
        if (code == 9) return "TAB";
        if (code == 10) return "LINE FEED (LF)";
        if (code == 13) return "CARRIAGE RETURN (CR)";
        if (code == 32) return "LEERZEICHEN";
